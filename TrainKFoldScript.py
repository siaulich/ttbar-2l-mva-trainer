...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of folds for k-fold cross-validation (optional)",
    )
    parser.add_argument(
        "--k_fold_index",
        type=int,
        default=0,
        help="Index of the fold to use for training (0-based, optional)",
    )
...
if __name__ == "__main__":
    ...
    if args.k_fold > 1 and args.k_fold_index > args.k_fold:
        raise ValueError(
            f"k_fold_index must be between 0 and k_fold-1 (got {args.k_fold_index} for k_fold={args.k_fold})"
        )
    X_train, y_train, X_test, y_test = data_preprocessor.get_k_fold(n_folds=args.k_fold, k_fold_idx=args.k_fold_index)
    num_events = X_train["jet_inputs"].shape[0]

    print(f"Data loaded. Using {num_events} events for training.")
    fold_idx = args.k_fold_index if args.k_fold > 1 else 0
    ...
    X_train, y_train, train_sample_weights = model.prepare_training_data(
        X_train, y_train, sample_weights=model.compute_sample_weights(X_train)
    )
    X_test, y_test, test_sample_weights = model.prepare_training_data(X_test, y_test, sample_weights=model.compute_sample_weights(X_test))

    train_options["validation_data"] = (X_test, y_test, test_sample_weights)
    train_options.pop("validation_split", None)
    ...
    if args.event_numbers == "even":
        model_name = f"model_fold{fold_idx + 1}_even"
    elif args.event_numbers == "odd":
        model_name = f"model_fold{fold_idx + 1}_odd"
    else:
        model_name = f"model_fold{fold_idx + 1}"
    model.save_model(os.path.join(args.output_dir, f"{model_name}.keras"))
    model.export_to_onnx(os.path.join(args.output_dir, f"{model_name}.onnx"))
